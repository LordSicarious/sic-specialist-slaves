...
public class ThoughtWorker_ExpectationsDutifulServant : ThoughtWorker {
	...
	public override string PostProcessDescription(Pawn p, string description) {
		return description.Formatted(ServantNoun(p.gender));
	}
	...
	private static string ServantNoun(Gender gender) {
		switch(gender) {
			case Gender.Male :
				return "Man".Translate();
			case Gender.Female :
				return "Woman".Translate();
			default :
				return "Servant".Translate();
		}
	}
	private static string ServantAdjective(Gender gender) {
		switch(gender) {
			case Gender.Male :
				return "Masculine".Translate();
			case Gender.Female :
				return "Feminine".Translate();
			default :
				return "Servant".Translate();
		}
	}
